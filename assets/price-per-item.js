  ...
  customElements.define(
    ...
    class PricePerItem extends HTMLElement {
      ...
      updatePricePerItem(updatedCartQuantity) {
        ...
        if (this.classList.contains('xboost-variant-item__price-per-item')) {
        ...
        for (let pair of this.qtyPricePairs) {
          if (this.currentQtyForVolumePricing >= pair[0]) {
            ...
            this.classList.contains('xboost-variant-item__price-per-item') ? pricePerItemCurrent.innerHTML = window.quickOrderListStrings.each.replace('[money]', pair[1]) : pricePerItemCurrent.innerHTML = pair[1];
            break;
          }
        }
      }
